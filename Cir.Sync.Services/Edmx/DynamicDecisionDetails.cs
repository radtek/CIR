//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Cir.Sync.Services.Edmx
{
    using System;
    using System.Collections.Generic;
    
    public partial class DynamicDecisionDetails
    {
        public int DecisionId { get; set; }
        public Nullable<int> Decision { get; set; }
        public string ImidiateActions { get; set; }
        public Nullable<int> RepeatedInspections { get; set; }
        public Nullable<int> AffectedBolts { get; set; }
        public System.DateTime CreatedDate { get; set; }
        public Nullable<System.DateTime> UpdatedDate { get; set; }
        public Nullable<bool> IsDeleted { get; set; }
        public long CirId { get; set; }
        public Nullable<int> FlangNo { get; set; }
        public Nullable<bool> FlangeDamageIdentified { get; set; }
        public string InspectionDesc { get; set; }
    
        public virtual ComponentInspectionReport ComponentInspectionReport { get; set; }
    }
}
